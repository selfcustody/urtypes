...
class Output(RegistryItem):
	...
	def descriptor(self):
		...
		keys = self.crypto_key.ec_keys[:] + self.crypto_key.hd_keys[:] if isinstance(self.crypto_key, MultiKey) else [self.crypto_key]
	...
	def from_data_item(cls, item):
		...
		is_multi_key = exp_len > 0 and (script_expressions[exp_len-1].expression == 'multi' or script_expressions[exp_len-1].expression == 'sortedmulti')
		if is_multi_key:
